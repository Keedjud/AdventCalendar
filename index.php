...
if (file_exists(SETTINGS_FILE)) {
	...
	if (isset($settings->month) && !empty($settings->month) && $settings->month > 1 && $settings->month <= 12) {
		define('MONTH', date('m', mktime(0, 0, 0, $settings->month + 0)));
	} else {
		define('MONTH', 12);
	}
	// is it an other begin day?
	if (isset($settings->first_day) && !empty($settings->first_day) && $settings->first_day > 0 && $settings->first_day <= 31) {
		define('FIRST_DAY', date('d', mktime(0, 0, 0, MONTH, $settings->first_day)));
	} else {
		define('FIRST_DAY', '01');
	}
	// is it an other last day?
	if (isset($settings->last_day) && !empty($settings->last_day) && $settings->last_day > FIRST_DAY && $settings->last_day <= 31) {
		define('LAST_DAY', date('d', mktime(0, 0, 0, MONTH, $settings->last_day)));
	} else {
		define('LAST_DAY', '24');
	}

	// is it a private calendar?
	if (isset($settings->passkey) && !empty($settings->passkey)) {
		define('PASSKEY', $settings->passkey);
	}
	...
	// want to rewrite URL?
	if (isset($settings->url_rewriting) && $settings->url_rewriting) {
		define('URL_REWRITING', TRUE);
	}
} else {
	die('<!doctype html><html><head><title>' . ADVENT_CALENDAR . '</title><style>body{width:600px;margin:50px auto 20px;}</style></head><body><div style="font-size:30px;"><strong>Oups!</strong> Settings file not found.</div><div><p>Edit <code>private/settings.example.json</code> to personnalize title and year and rename it <code>settings.json</code>.</p><p>If it is not already done, put your photos in the <code>private/</code> folder, and name them with the number of the day you want to illustrate.</p></div></body></html>');
}
...
if (!is_file(PRIVATE_FOLDER . '/.htaccess')) {
	file_put_contents(PRIVATE_FOLDER . '/.htaccess', 'Require all denied');
}
...
abstract class AddOns
{
	...
	{
		if (empty($addon[self::Data])) {
			$addon[self::Data] = TRUE;
		}
...
abstract class Routes
{
	...
	static function initialize()
	{
		if (empty(self::$base)) {
			...
			if (strcasecmp(substr(self::$base, strlen(self::$base) - 1), '/') !== 0) {
				self::$base .= '/';
			}
...
abstract class Advent
{
	...
	static function state()
	{
		...
		if ($now < YEAR . MONTH . FIRST_DAY) {
			return self::BEFORE_ADVENT;
		}
		// if we are after
		if ($now > YEAR . MONTH . LAST_DAY) {
			return self::AFTER_ADVENT;
		}
	...
	static function getDaysHtml()
	{
		...
		foreach (self::getDays() as $d) {
			if ($d->active) {
				$result .= '<a href="' . $d->url . '" title="' . I18n::translation('day-d', $d->day) . '"';
			} else {
				$result .= '<div';
			}
	...
	static function getDay($day)
	{
		...
		if (file_exists(CALENDAR_FILE)) {
			...
			if (!empty($file->{$day})) {
				if (!empty($file->{$day}->title)) {
					$title = htmlspecialchars($file->{$day}->title);
				}
				if (!empty($file->{$day}->legend)) {
					$legend = htmlspecialchars($file->{$day}->legend);
				}
				if (!empty($file->{$day}->text)) {
					$text = $file->{$day}->text;
				}
				if (!empty($file->{$day}->link)) {
					$link = $file->{$day}->link;
				}
			}
		}
	...
	static function getDayHtml($day)
	{
		...
		// do we have a text?
		if (!empty($text)) {
			$result .= '<div class="text panel panel-default"><div class="panel-body">' . $text . '</div></div>';
		}

		// we do not forget the pagination
		$result .= '<ul class="pager"><li class="previous';
		if (self::isActiveDay($day - 1) && ($day - 1) >= FIRST_DAY) {
			$result .= '"><a href="' . Routes::route(URL_DAY, $day - 1) . '" title="' . I18n::translation('previous-link-title') . '" class="tip" data-placement="right">';
		} else {
			$result .= ' disabled"><a>';
		}
		$result .= '<i class="glyphicon glyphicon-hand-left"></i></a></li><li class="next';
		if (self::isActiveDay($day + 1) && ($day + 1) <= LAST_DAY) {
			$result .= '"><a href="' . Routes::route(URL_DAY, $day + 1) . '" title="' . I18n::translation('next-link-title') . '" class="tip" data-placement="left">';
		} else {
			$result .= ' disabled"><a>';
		}
...
// want to see a photo ?
else if (isset($_GET[URL_PHOTO])) {
	Image::get($_GET[URL_PHOTO] + 0);
}
...
if (isset($_GET[URL_ABOUT])) {
	// if ugly URL
	if (!empty($_GET[URL_ABOUT])) {
		header('Location: ' . Routes::route(URL_ABOUT));
		exit();
	}
...
<head>
	...
	<?php if (!defined('PASSKEY')): ?>
		<link rel="alternate" type="application/rss+xml" href="<?php echo Routes::route(URL_RSS); ?>" title="<?php echo TITLE; ?>" /><?php endif; ?>
	...
	<?php if (AddOns::Found('js')): ?>
		<script>
			...
			<?php if (AddOns::Found('piwik')): $piwik = AddOns::Get('piwik'); ?>
				var _paq = _paq || [];
				(function() {
					var u = '//<?php echo $piwik["piwik_url"]; ?>/';
					_paq.push(['setSiteId', '<?php echo $piwik["site_id"]; ?>']);
